...
{if $gBitSystem->isPackageActive( 'ilike' )}
	{bitmodule title="$moduleTitle" name="ilike_box"}
		{form method="get" ipackage=ilike ifile="index.php"}
			<div class="row">
				<input id="fuser" name="highlight" size="20" type="text" accesskey="s" value="{tr}Search{/tr}" onblur="if (this.value == '') {ldelim}this.value = '{tr}Search{/tr}';{rdelim}" onfocus="if (this.value == '{tr}Search{/tr}') {ldelim}this.value = '';{rdelim}" />
				<br />
